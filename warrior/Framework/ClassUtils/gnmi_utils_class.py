        ...
        while 1:
            try:
                ...
                if u_index == 0:
                    ...
                    testcase_Utils.pNote(child.before.decode("utf-8")+child.match.group(0).decode("utf-8"))
                if u_index == 1:
                    status = False
                    testcase_Utils.pNote(child.before+child.match.group(0).decode("utf-8")+child.after.decode("utf-8"))
                    break
                if u_index == 2:
                    testcase_Utils.pNote(child.before.decode("utf-8"))
                    if "100%" in child.before.decode("utf-8"):
                        status =True
                    break
                if u_index == 4:
                    testcase_Utils.pNote(child.before.decode("utf-8")+child.match.group(0).decode("utf-8"))
                    if "100%" in child.before.decode("utf-8")+child.match.group(0).decode("utf-8"):
                        status =True
                    break
                if u_index == 3:
                    testcase_Utils.pNote(child.before+child.match.group(0).decode("utf-8")+child.after.decode("utf-8"))
    ...
    def execuate(self, cmd_string, uname, passwd, external_system=None,
        ...
        if script.lower() == "no":
            ...
            if u_index == 0:
                ...
                status = True
                testcase_Utils.pNote(child.match.group(0).decode("utf-8") + uname)
                excute = True
            else:
                excute = False
            p_index = child.expect(["password.*", ".*error.*", pexpect.EOF, pexpect.TIMEOUT],
                                   timeout=5)
            if p_index == 0:
                testcase_Utils.pNote(child.match.group(0).decode("utf-8"))
                child.sendline(passwd)
                excute = True
            else:
                excute = False
        if excute or script.lower() == "yes":
            if stop_after == None and "polling" not in cmd_string and "streaming" not in cmd_string:
                ...
                if j_index == 1 or j_index == 0:
                    if "client had error while displaying results" not in child.before.decode("utf-8"):
                        ...
                        status = False
                        result = child.before.decode("utf-8")
                        testcase_Utils.pNote(child.before.decode("utf-8"), "error")
            if stop_after: #For polling or streaming
                ...
                if "client had error while displaying results" not in child.before:
                    result = child.before.decode("utf-8").strip().strip("^Z")
                    testcase_Utils.pNote(result)
                status = True
    ...
    def close(self, system_name=None, external_system=None, external_system_session=None):
        ...
        if "client had error while displaying results" not in gnmi_obj.before.decode("utf-8"):
            if system_name:
                ...
                result = gnmi_obj.after.decode("utf-8").strip().strip("^C")
                testcase_Utils.pNote(result)
            status = True
