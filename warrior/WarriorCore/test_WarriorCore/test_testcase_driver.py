...
from warrior.WarriorCore import testcase_driver, common_execution_utils
from warrior.Framework.Utils import config_Utils, xml_Utils, testcase_Utils, file_Utils, string_Utils
from warrior.WarriorCore import framework_detail,  defects_driver
from warrior.WarriorCore.Classes import hybrid_driver_class,  execution_files_class,  execution_files_class
...
def test_get_testcase_details():
    ...
    assert result['wt_filename']== None
    assert result['wt_filedir']== None
    ...
    assert result['wt_kw_results_dir']== None
    assert result['wt_defectsdir']== 8
    assert result['wt_console_logfile']== None
    assert result['wt_expResults']== 9
    assert result['wt_operating_system']== 'LINUX'
    assert result['wt_def_on_error_action']== 'NONE'
    assert result['wt_def_on_error_value']== None
    assert result['jiraproj']== None
