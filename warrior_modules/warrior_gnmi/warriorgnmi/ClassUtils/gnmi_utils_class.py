...
import json, re
from warrior.Framework.Utils import testcase_Utils, data_Utils, config_Utils, file_Utils
...
class gnmi(object):
    ...
    def execute(self, binary, cmd_string, uname, passwd, prompt, external_system=None,
        ...
        if script.lower() == "no":
            ...
            for response, value in credentials.items():
            ...
            if stop_after == None and "poll" not in cmd_string.lower() \
                ...
                if j_index == 1 or j_index == 0:
                    if "client had error while displaying results" not in (child.before).decode('utf-8') \
                           and "could not create a gNMI client" not in (child.before).decode("utf-8"):
                        if j_index == 1:
                            sleep(5)
                            result = child.before
