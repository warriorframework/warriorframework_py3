...
"""
templates_dir = os.path.join(os.path.abspath(NAV.get_katana_dir()),
                             'wapps',
                             'execution',
                             'templates')
data_live_dir = os.path.join(os.path.abspath(NAV.get_katana_dir()),
                             'wapps',
                             'execution',
                             '.data',
                             'live')
...
class Execution(object):
    ...
    def __init__(self):
        ...
        self.warrior = os.path.join(os.path.abspath(NAV.get_warrior_dir()),
                                    'Warrior')
        self.default_ws = os.path.join(self.wf_dir,
                                       'warrior',
                                       'Warriorspace')
        self.templates_dir = os.path.abspath(os.path.join(templates_dir,
                                                          'execution'))
        self.jira_settings_file = os.path.join(self.wf_dir,
                                               'warrior',
                                               'Tools',
                                               'jira',
                                               'jira_config.xml')
        self.execution_settings_json = os.path.join(templates_dir,
                                                    'execution',
    ...
    def index(self, request):
        ...
        # testing get_all_wf_versions
        all_wf_versions = NAV.get_all_wf_versions()
        print(all_wf_versions)
        ...
        start_dir = self.default_ws if execution_settings_dict['defaults'][
            'start_dir'] == 'default' else execution_settings_dict['defaults']['start_dir']
        execution_settings_dict['defaults']['start_dir'] = start_dir
        index_template = os.path.abspath(
            os.path.join(
                self.templates_dir,
                'execution.html'))
    ...
    def get_results_index(self, request):
        ...
        results_template = os.path.join(
            self.templates_dir, 'execution_results.html')
    ...
    def execute_warrior(self, request):
        ...
        return StreamingHttpResponse(stream_warrior_output(self.warrior,
                                                           cmd_string,
                                                           execution_file_list,
                                                           live_html_res_file,
                                                           python_path))
...
def stream_warrior_output(warrior_exe,
                          cmd_string,
                          file_list,
                          live_html_res_file,
                          python_path=None):
    ...
    warrior_cmd = '{0} {1} -livehtmllocn {2} {3}'.format(pypath,
                                                         warrior_exe,
                                                         live_html_res_file,
                                                         cmd_string)
