var suites = {
    ...
    mappings: {
        /* titles from drawers */
        newStep: {
            title: "New Case" //drawer title for when new step is being edited
        },
        newReq: {
        ...
        editStep: {
            title: "Edit Case" //drawer title for when existing step is being edited
        }
    },
        ...
        open: {
            ...
            switchView: {
                ...
                details: function ($container, data) {
                    ...
                    suites.generateDetailsDisplayHtmlBlock($container, data);
                    katana.$activeTab.find('#details_drawer_template').parent().prepend(suites.proofDetailsContainer($container));
                },
                ...
                steps: function($container, data, stepNum) {
                    ...
                    if (data) {
                        $container = suites.generateStepsHtmlBlock($container, data);
                    } else {
                        $container = suites.addTsToStepsHtmlBlock($container, stepNum);
                    }
                    katana.$activeTab.find('#steps_drawer_template').parent().prepend(suites.proofStepsContainer($container));
                }
            },
            ...
            saveContents: function () {
                ...
                if ($switchElem) {
                    ...
                    if (reference === "editDetails") {
                        saved = suites.drawer.open._saveContents.details($openDrawer.find('.content').find('#current_details_editor'));
                    } else if (reference === "newReq") {
                        saved = suites.drawer.open._saveContents.requirements($openDrawer.find('.content').find('#current_reqs_editor'));
                    } else if (reference === "newStep") {
                        saved = suites.drawer.open._saveContents.steps($openDrawer.find('.content').find('#current_steps_editor'));
                    }
                    if (saved) {
                        $openDrawer.find('.suites-drawer-open-body').find('.content').find('[id^="current"]').remove();
                        suites.drawer.closeOpenedDrawer();
                    }
                }
            },
            ...
            _saveContents: {
                ...
                details: function ($container) {
                    ...
                    if (katana.validationAPI.init($container)) {
                        var data = suites.generateJson.generateDetails($container);
                        var displayContent = suites.generateDetailsDisplayHtmlBlock(katana.$activeTab.find('#suites-display-template').clone(), data);
                ...
                requirements: function ($container) {
                    ...
                    if (katana.validationAPI.init($container)) {
                        var data = suites.generateJson.generateRequirements(katana.$activeTab.find('#suites-requirements-template'));
                        var temp = suites.generateJson.generateRequirements($container);
                        ...
                        var displayContent = suites.generateRequirementsHtmlBlock(katana.$activeTab.find('#suites-requirements-template').clone(), data);
                ...
                steps: function ($container) {
                    ...
                    if (katana.validationAPI.init($container)) {
                        var data = suites.generateJson.generateStep($container);
                        var displayContent = suites.generateStepsDisplayHtmlBlock(katana.$activeTab.find('#step-row-template').clone().attr('id', ''), data);
    ...
    header: {
        ...
        editDetails: function () {
            ...
            var $elem = $(this);
            var data = suites.generateJson.generateDetails($elem.closest('#main-div').find('#detail-block'));
            suites.drawer.openClosedDrawer(suites.mappings.editDetails.title);
            suites.drawer.open.highlightSidebar(0);
            suites.drawer.open.switchView.details($elem.closest('#main-div').find('#details_drawer_template').clone(), data);
        },

        newReq: function() {
            /* This function creates a new requirements block in the side drawer*/
            var $elem = $(this);
            suites.drawer.openClosedDrawer(suites.mappings.newReq.title);
            suites.drawer.open.highlightSidebar(1);
            suites.drawer.open.switchView.requirements($elem.closest('#main-div').find('#reqs_drawer_template').clone());
        },

        newStep: function() {
            /* This function creates a new case-step block in the side drawer*/
            var $elem = $(this);
            suites.drawer.openClosedDrawer(suites.mappings.newStep.title);
            suites.drawer.open.highlightSidebar(2);
            var $container = $elem.closest('#main-div').find('#steps_drawer_template').clone().attr('step-type', 'edit').attr('index', suites.getLastStepNum());
            suites.drawer.open.switchView.steps($container, false, suites.getLastStepNum() + 1);
        }
        ...
        openNewFile: function () {
            $.ajax({
                ...
                if(data.status){
                    ...
                    katana.$activeTab.find('.tool-bar').find('.title').html(data.filename);
                    katana.$activeTab.find('#main-div').html(data.html_data);
                    suites.mappings.newStep.drivers = data.drivers;
                } else {
                    katana.openAlert({"alert_type": "danger",
                        "heading": "Could not open file",
                        "text": "Errors: " + data.message,
    ...
    suiteViewer: {
        ...
        save: function () {
            ...
            var final_json = {"TestSuite": {}};
            var filepath = katana.$activeTab.find('#main-div').attr("current-file");
            var filename = katana.$activeTab.find('.tool-bar').find('.title').html();
            $.extend(true, final_json.TestSuite, suites.generateJson.generateDetails(katana.$activeTab.find('#detail-block')));
            $.extend(true, final_json.TestSuite, suites.generateJson.generateRequirements(katana.$activeTab.find('#req-block')));
            $.extend(true, final_json.TestSuite, suites.generateJson.generateStep(katana.$activeTab.find('#step-block')));
    ...
    generateJson: {
        generateDetails: function ($container) {
            ...
            for (var i = 0; i < $allKeys.length; i++) {
                ...
                $.extend(true, finalJson.Details, suites.generateJson._updateJson($($allKeys[i]).attr("key").trim(), value));
            }
            return finalJson;
        },
        ...
        generateStep: function ($container) {
            ...
            var finalJson = {Testcases: { Testcase: []}};
            var $allTcs = $container.attr('key') === 'Testcase'? [$container] : $container.find('[key="Testcase"]');
            var $allKeys = false;
            ...
            for (var i=0; i <$allTcs.length; i++) {
                ...
                for (var j=0; j<$allKeys.length; j++) {
                    ...
                    $.extend(true, partialData, suites.generateJson._updateJson(key, value));
                }
                $.extend(true, partialData, suites.generateJson.generateExecuteRules($($allTcs[i]).find('[key="Execute.Rule"]')));
                finalJson.Testcases.Testcase.push(partialData);
            }
            return finalJson
        },
        ...
        _updateJson: function (unrefined_key, value) {
            ...
            if (key !== undefined && remaining_key !== undefined) {
                data[key] = suites.generateJson._updateJson(remaining_key, value)
            } else {
    ...
    getValueFromJson: function (data, unrefined_key) {
        ...
        var remaining_key = unrefined_key.split(/\.(.+)/)[1];
        if (key !== undefined && remaining_key !== undefined) {
            return suites.getValueFromJson(data[key], remaining_key)
        }
        return data[key]
    },
    ...
    generateDetailsDisplayHtmlBlock: function ($container, data) {
        ...
        for (var i = 0; i < $allKeys.length; i++) {
            if ($($allKeys[i]).prop('type') === 'text') {
                $($allKeys[i]).attr('value', suites.getValueFromJson(data.Details, $($allKeys[i]).attr('key')));
            } else if ($($allKeys[i]).prop('type') === 'select-one') {
                $($allKeys[i]).attr('value', suites.getValueFromJson(data.Details, $($allKeys[i]).attr('key')));
                var value = suites.getValueFromJson(data.Details, $($allKeys[i]).attr('key'));
                ...
                $($allKeys[i]).html(suites.getValueFromJson(data.Details, $($allKeys[i]).attr('key')));
            }
        }
        return $container
    },
    ...
    generateRequirementsHtmlBlock: function ($container, data) {
        ...
        var $tableTemplateLeft = suites._generateReqTable(data, start, mid, $constantTemplate, $constantTemplate.clone());
        var $tableTemplateRight = suites._generateReqTable(data, mid, end, $constantTemplate, $constantTemplate.clone());
    ...
    generateStepsHtmlBlock: function ($container, data) {
        ...
        for (var i=0; i<$allKeys.length; i++) {
            if ($($allKeys[i]).prop('type') === 'text') {
                $($allKeys[i]).attr('value', suites.getValueFromJson(data.Testcases.Testcase[0], $($allKeys[i]).attr('key')));
            } else if ($($allKeys[i]).prop('type') === 'select-one') {
                $($allKeys[i]).attr('value', suites.getValueFromJson(data.Testcases.Testcase[0], $($allKeys[i]).attr('key')));
                var value = suites.getValueFromJson(data.Testcases.Testcase[0], $($allKeys[i]).attr('key'));
                var $allOptions = $($allKeys[i]).children();
                for (var j=0; j<$allOptions.length; j++) {
                    if ($($allOptions[j]).text() === value) {
                        $($allOptions[j]).prop('selected', true);
                ...
                $($allKeys[i]).html(suites.getValueFromJson(data.Testcases.Testcase[0], $($allKeys[i]).attr('key')));
            }
        }
        $container = suites._addExecuteBlock($container, data);
        return $container
    },
    ...
    _addExecuteBlock: function ($container, data) {
        ...
        $execTypeBlock.attr('current-value', suites.getValueFromJson(data.Testcases.Testcase[0], $execTypeBlock.attr('key')));
        suites._selectOption($execTypeBlock, $execTypeBlock.attr('current-value'));

        var $ruleBlock = $execTypeBlock.closest('.row').next().find('#rule-template');
        for (var k=0; k<data.Testcases.Testcase[0].Execute.Rule.length; k++) {
            var $currentRuleBlock = $ruleBlock.clone().attr('id', '').attr('key', 'Execute.Rule').show();
            var $allKeys = $currentRuleBlock.find('[key]');
            for (var i=0; i<$allKeys.length; i++) {
                var value = suites.getValueFromJson({"Execute" : {"Rule": data.Testcases.Testcase[0].Execute.Rule[k]}}, $($allKeys[i]).attr('key'));
                $($allKeys[i]).attr('value', value);
                if ($($allKeys[i]).prop('type') === 'select-one') {
                    suites._selectOption($($allKeys[i]), value);
                }
            }
            suites.stepEditor.updateRuleElseValue($currentRuleBlock.find('[key="Execute.Rule.@Else"]'));
    ...
    generateStepsDisplayHtmlBlock: function ($container, data) {
        ...
        $container = suites.evaluateDisplayRunmode($container);
        $container = suites.evaluateDisplayOnError($container);
        $container = suites.evaluateDisplayExecute($container, data);
        return $container;
    },
    ...
    evaluateDisplayExecute: function ($container, data) {
        ...
        for (var i=0; i<data.Testcases.Testcase[0].Execute.Rule.length; i++) {
            var $currentTemplate = $templateExecRule.clone();
            var $allKeys = $currentTemplate.find('[key]');
            for (var j=0; j<$allKeys.length; j++) {
                $($allKeys[j]).html(suites.getValueFromJson({"Execute": {"Rule": data.Testcases.Testcase[0].Execute.Rule[i]}}, $($allKeys[j]).attr('key')));
            }
    ...
    proofDetailsContainer: function ($container) {
        ...
        suites.detailsEditor.updateDefaultOnError($container.find('[key="default_onError.@action"]'));
        suites.detailsEditor.updateExecutionType($container.find('[key="type.@exectype"]'));
        return $container;
    },
    ...
    proofStepsContainer: function ($container) {
        ...
        suites.stepEditor.updateRules($container.find('[key="Execute.@ExecType"]'));
        suites.stepEditor.updateRunmode($container.find('[key="runmode.@type"]'));
        suites.stepEditor.updateOnErrorValue($container.find('[key="onError.@action"]'));
        return $container
    },
    ...
    detailsEditor: {
        ...
        updateDefaultOnError: function ($elem) {
            ...
            $elem = $elem ? $elem : $(this);
            suites._updateCommonOnError($elem);
        },
    ...
    reqSection: {
        deleteReq: function () {
            ...
            katana.openAlert({
                ...
                $toBeDeleted.remove();
                var data = suites.generateJson.generateRequirements($topLevel);
                var displayContent = suites.generateRequirementsHtmlBlock($topLevel.clone(), data);
        ...
        editReq: function () {
            ...
            suites.drawer.openClosedDrawer(suites.mappings.editReq.title);
            suites.drawer.open.highlightSidebar(1);
            suites.drawer.open.switchView.requirements($elem.closest('#main-div').find('#reqs_drawer_template').clone(), data, $elem.closest('tr').attr('req-number')-1);
        }
    },
    ...
    stepSection: {
        ...
        toolbar: {
            ...
            insertStep: function () {
                ...
                suites.drawer.open.switchView.steps($container, false, insertAtIndex+1);
            },
            ...
            editStep: function () {
                ...
                if ($allTrElems.length === 0) {
                    ...
                    var data = suites.generateJson.generateStep($($allTrElems[0]));
                    suites.drawer.openClosedDrawer(suites.mappings.editStep.title);
                    suites.drawer.open.highlightSidebar(2);
                    var $container = $elem.closest('#main-div').find('#steps_drawer_template').clone().attr('step-type', 'edit').attr('index', $($allTrElems[0]).index());
                    suites.drawer.open.switchView.steps($container, data);
                }
            }
        }
    },
